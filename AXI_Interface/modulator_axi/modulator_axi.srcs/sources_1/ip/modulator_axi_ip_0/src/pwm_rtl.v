...
module pwm # (
    parameter width_p = 10'd12          // the number of bits used to represent ampliute value
)(
...
// threshold_r is variable taht is telling us when pwm signal should be changed from 1 to 0
// integer range 0 to 4095 (in our case)
reg [12:0] threshold_r = 20'd0;
...
begin
    if (ce_w == 1)
        ...
        begin
            ...
            threshold_r = sine_ampl;
            count_r = 20'd0;        // default assignment
            
            // if current amplitude of the sine signal is greater than zero,
            // there will be a pulse on the PWM signal in the current period
            // (PWM will be 1 for a period of time)
            if (sine_ampl > 0)
                state_r <= pwm_high;
                
            // if current amplitude value is equal to zero,
            // there will be no pulse on the PWM signal in the current period
            // (PWM will always be 0)
            else if (sine_ampl == 0)
                state_r <= pwm_low;
        end
        ...
        begin
            ...
            if ((count_r < ((2**width_p)-1)) && (count_r < threshold_r))
                state_r <= pwm_high;
            ...
            else if (count_r < ((2**width_p)-1) && (count_r == threshold_r))
                state_r <= pwm_low;
        end
